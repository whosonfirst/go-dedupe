...
cli:
	go build -tags sqlite,sqlite_vec,duckdb,ollama -mod $(GOMOD) -ldflags="$(LDFLAGS)" -o bin/compare-locations cmd/compare-locations/main.go
	go build -tags sqlite,sqlite_vec,duckdb,ollama -mod $(GOMOD) -ldflags="$(LDFLAGS)" -o bin/index-locations cmd/index-locations/main.go
